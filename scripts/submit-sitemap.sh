...
# Proxy settings for accessing Google/Bing from China
...
SITEMAP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 15 --max-time 30 "$SITEMAP_URL" 2>/dev/null)
...
RESPONSE=$(curl -s -x socks5h://127.0.0.1:7890 -o /dev/null -w "%{http_code}" --connect-timeout 15 --max-time 30 "$GOOGLE_PING" 2>/dev/null)
...
BING_RESPONSE=$(curl -s -x socks5h://127.0.0.1:7890 -o /dev/null -w "%{http_code}" --connect-timeout 15 --max-time 30 "$BING_PING" 2>/dev/null)
